(ns mobileye.main
  "Kotodama WASM entrypoint for the Mobileye clean-room actor (L4) — Clojure port.
  ...
  facts (`mobileye.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mobileye")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mobileye.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mobileye-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mobileye-compat/ready)
